IsoMap = (function() {
    ...
    IsoMap.prototype.addListeners = function() {
        ...
        this.canvas.addEventListener('mousedown', function onMouseDown(event) {
            ...
            if (isOnMap(isometricPosition, self.map)) {
				self.tileArray.push(isometricPosition);
				self.tileArray.sort((a, b) => a.x - b.x || a.y - b.y);
				self.tileArray.forEach(element => {
					self.drawPrism(element);
				});
    ...
    IsoMap.prototype.convertScreenToIsometric = function(x, y) {
        x = (x - (this.position.x - this.tile.width / 2)) / this.tile.width;
